...
const ftnApi = new Fortnite("017d46de-0b9f-458b-90ac-41c9182eeb82");
...
exports.run = async (client, message, args, ops) => {
    ...
    if (args[2]) {
        if (args[2].toLowerCase() == "all" || args[2].toLowerCase() == "season") {
            ...
            return message.channel.send(
                "Erreur. Utilisez la bonne syntaxe: `/fortnite <epic-username> [platform pc/xbl/psn] {mode all/season}`.\nPour les statistiques de durée de vie, utilisez `/fortnite <epic-username> [platform pc/xbl/psn]`"
            ); //Sends error message
        }
    }
    ...
    if (!username)
        ...
        return message.channel.send(
            "Nom d'utilisateur non fourni. Utilisez la bonne syntaxe: `/fortnite <epic-username> [platform pc/xbl/psn] {mode all/season}`.\nPour les statistiques de durée de vie, utilisez `/fortnite <epic-username> [platform pc/xbl/psn]`"
        ); //Sends error message
    ...
    let data = ftnApi
        ...
        .then(data => {
            ...
            if (mode == "life") {
                ...
                let lifeEmbed = new Discord.RichEmbed()
                    ...
                    .addField("Pourcentage de victoire", lifeWinPercent, true)
            ...
            if (mode.toLowerCase() == "all") {
                ...
                let duoEmbed = new Discord.RichEmbed()
                    ...
                    .setDescription(`Duo stats pour ${data.username}`)
                    .setColor("RANDOM")
                ...
                let squadEmbed = new Discord.RichEmbed()
                    ...
                    .setDescription(`Statistiques de l'équipe pour ${data.username}`)
                    .setColor("RANDOM")
            ...
            if (mode.toLowerCase() == "season") {
                ...
                let currentSoloEmbed = new Discord.RichEmbed()
                    ...
                    .setThumbnail("https://s3.amazonaws.com/media.atp/42511_solof.png")
                    .setDescription(
                        `Saison ${currentSeason} Statistiques solo pour ${data.username}`
                    )
                    .setColor("RANDOM")
                ...
                let currentDuoEmbed = new Discord.RichEmbed()
                    ...
                    .setDescription(
                        `Saison ${currentSeason} Duo stats pour ${data.username}`
                    )
                    .setColor("RANDOM")
                ...
                let currentSquadEmbed = new Discord.RichEmbed()
                    ...
                    .setDescription(
                        `Saison ${currentSeason} Statistiques de l'équipe pour ${data.username}`
                    )
                    .setColor("RANDOM")
        ...
        .catch(e => {
            ...
            //console.log(e);
            return message.channel.send(
                "Erreur. Utilisateur non trouvé, assurez-vous d'utiliser la bonne syntaxe: `/fortnite <epic-username> [platform pc/xbl/psn] {mode all/season}`.\nPour les statistiques de durée de vie, utilisez `/fornite <epic-username> [platform pc/xbl/psn]`\n\n**Si ce problème persiste, assurez-vous d'utiliser le `.issue` commande pour signaler tout problème avec le bot**"
            ); //Send error message
        });
};